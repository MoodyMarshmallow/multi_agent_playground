...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # Handle TOKEN LIMIT EXCEEDED case
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
        # Return a simple single-task decomposition based on the original task
        if "sleep" in task.lower():
            return [["sleeping", duration]]
        elif "eat" in task.lower():
            return [["eating", duration]]
        elif "work" in task.lower():
            return [["working", duration]]
        elif "study" in task.lower():
            return [["studying", duration]]
        else:
            # Generic fallback - break into smaller chunks
            if duration > 60:
                chunk_size = 30
                chunks = []
                remaining = duration
                while remaining > 0:
                    ...
                    chunks.append([task.split("(")[0].strip() if "(" in task else task, chunk_duration])
    ...
    ret = []
    for i in cr: 
      # ret += [[i.split(" (duration in minutes: ")[0], 
      #          int(i.split(" (duration in minutes: ")[1])]]
  ...
  def __func_validate(gpt_response, prompt=""): 
    # Check for TOKEN LIMIT EXCEEDED case, which we'll handle in clean_up
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
      return True
      
    # Check if response can be parsed properly
    try: 
...
def run_gpt_prompt_action_arena(action_description, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
        # Get available arenas and return the first one
        x = f"{act_world}:{act_sector}"
        accessible_arenas = persona.s_mem.get_str_accessible_sector_arenas(x)
        if accessible_arenas:
            first_arena = accessible_arenas.split(", ")[0].strip()
            return first_arena
        return "main room"  # Default fallback
    cleaned_response = gpt_response.strip()
    return cleaned_response

  def __func_validate(gpt_response, prompt=""): 
    # Special case for TOKEN LIMIT EXCEEDED
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
      return True
    try: __func_clean_up(gpt_response, prompt="")
    except: return False
  ...
  def get_fail_safe(): 
    # Try to get the first available arena from spatial memory
    x = f"{act_world}:{act_sector}"
    accessible_arenas = persona.s_mem.get_str_accessible_sector_arenas(x)
    if accessible_arenas:
        first_arena = accessible_arenas.split(", ")[0].strip()
  ...
  # Validate that the output arena actually exists in the spatial memory
  x = f"{act_world}:{act_sector}"
  available_arenas = [i.strip() for i in persona.s_mem.get_str_accessible_sector_arenas(x).split(",")]
  available_arenas = [arena for arena in available_arenas if arena]  # Remove empty strings
  
  if output not in available_arenas and available_arenas:
    # If the generated arena doesn't exist, pick the first available one
    output = available_arenas[0]
    print(f"Arena validation failed, using fallback: {output}")
...
def run_gpt_prompt_action_game_object(action_description, 
  ...
  def __func_validate(gpt_response, prompt=""): 
    if len(gpt_response.strip()) < 1: 
      return False
    # Accept TOKEN LIMIT EXCEEDED as a special case that will be handled in cleanup
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
      return True
    return True
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # Handle TOKEN LIMIT EXCEEDED
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
        # Try to get available objects and return appropriate one based on action
        available_objects = persona.s_mem.get_str_accessible_arena_game_objects(temp_address)
        if available_objects:
            objects = [obj.strip() for obj in available_objects.split(",")]
            objects = [obj for obj in objects if obj]  # Remove empty strings
            
            # Choose object based on action description
            action_lower = action_description.lower()
            if "sleep" in action_lower:
                for obj in objects:
                    if "bed" in obj.lower():
                        return obj
            elif "eat" in action_lower or "cook" in action_lower:
                for obj in objects:
                    if any(word in obj.lower() for word in ["stove", "fridge", "kitchen", "counter"]):
                        return obj
            elif "work" in action_lower or "study" in action_lower:
                for obj in objects:
                    if any(word in obj.lower() for word in ["desk", "computer", "chair"]):
                        return obj
            
            # If no specific match, return first available object
            if objects:
                return objects[0]
        return "bed"  # Final fallback
  ...
  # Validate that the output object exists in the available objects
  available_objects_str = persona.s_mem.get_str_accessible_arena_game_objects(temp_address)
  if available_objects_str:
    x = [i.strip() for i in available_objects_str.split(",")]
    x = [obj for obj in x if obj]  # Remove empty strings
    
    if output not in x and x:
      # Try to find a reasonable fallback based on action description
      action_lower = action_description.lower()
      ...
      # If no semantic match found, use random choice
      if not found_fallback:
          output = random.choice(x)
      
      print(f"Object validation failed, using fallback: {output}")
  else:
    # No objects available, use a generic fallback
    output = "furniture"
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # Handle TOKEN LIMIT EXCEEDED
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
        return ["is", "idle"]  # Return default predicate-object pair
    cr = gpt_response.strip()
    cr = [i.strip() for i in cr.split(")")[0].split(",")]
    return cr

  def __func_validate(gpt_response, prompt=""): 
    # Accept TOKEN LIMIT EXCEEDED as valid
    if "TOKEN LIMIT EXCEEDED" in gpt_response:
        return True

  ...
  if not persona.scratch.act_path_set: 
    ...
    if "<persona>" in plan: 
      ...
      if plan not in maze.address_tiles: 
        ...
        print(f"Warning: Address '{plan}' not found in maze.address_tiles")
        
        # Try to extract components and find a fallback
        try:
          ...
          if len(plan_parts) >= 3:
            ...
            # Try to find an existing address in the same sector
            fallback_address = None
            for addr in maze.address_tiles.keys():
              addr_parts = addr.split(":")
              if len(addr_parts) >= 2 and addr_parts[0] == world and addr_parts[1] == sector:
                fallback_address = addr
                break
            
            if fallback_address:
              print(f"Using fallback address: {fallback_address}")
              target_tiles = maze.address_tiles[fallback_address]
            else:
              # Use the persona's current tile as a last resort
              print(f"No fallback found, staying at current location")
              target_tiles = [persona.scratch.curr_tile]
          else:
            # Invalid plan format, stay at current location
            print(f"Invalid plan format: {plan}")
            target_tiles = [persona.scratch.curr_tile]

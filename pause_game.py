#!/usr/bin/env python3
"""
Terminal command to pause the multi-agent playground server.
Usage: python pause_game.py
"""
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            if proc.info['name'] and 'python' in proc.info['name'].lower():
                ...
                if cmdline and any('backend.interfaces.http.main:app' in arg or 'backend/interfaces/http/main.py' in arg for arg in cmdline):
...
def pause_game(port):
    ...
    try:
        print(f"Sending pause request to port {port}...")
        response = requests.post(f"http://localhost:{port}/game/pause", timeout=5)
        ...
        elif status == "already_paused":
            print("ℹ Game is already paused")
        else:
...
if __name__ == "__main__":
    ...
    if port is None:
        ...
        print("    python -m uvicorn backend.interfaces.http.main:app --reload")
        print("    or")
